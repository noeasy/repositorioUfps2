...
<dspace:layout style="submission" titlekey="jsp.dspace-admin.authorize-community-edit.title"
    ...
    <dspace:popup page="<%= LocaleSupport.getLocalizedMessage(pageContext, \"help.site-admin\") + \"#communitypolicies\"%>"><i class="fas fa-question-circle fa-xs"></i></dspace:popup>
    </h1>
            ...
            <td headers="t3" class="<%= row %>RowOddCol">
             ...
             <td headers="t4" class="<%= row %>RowEvenCol">
                <form action="<%= request.getContextPath() %>/tools/authorize" method="post">
                    ...
                    <input type="hidden" name="community_id" value="<%= community.getID() %>" />
		    <button class="btn btn-default" type="submit" name="submit_community_edit_policy"> <i class="fas fa-edit"></i> </button>
                </form>
             </td>
             <td headers="t5" class="<%= row %>RowOddCol">
                <form action="<%= request.getContextPath() %>/tools/authorize" method="post">
                    <input type="hidden" name="policy_id" value="<%= rp.getID() %>" />
                    <input type="hidden" name="community_id" value="<%= community.getID() %>" />
		    <button class="btn btn-danger" type="submit" name="submit_community_delete_policy"> <i class="fas fa-trash-alt"></i></button>
                </form>
             </td>
         </tr>
